...
from adapteacher.data.detection_utils import build_strong_augmentation, build_strong_augmentation_detect
...
class DatasetMapperTwoCropSeparate_detect(DatasetMapper):
    """
    This customized mapper produces two augmented images from a single image
    instance. This mapper makes sure that the two augmented images have the same
    cropping and thus the same size.
    ...
    def __init__(self, cfg, is_train=True, keep_tf_data=False):
        if 1:#cfg.SEMISUPNET.USE_DINO or 'Trunk' in cfg.MODEL.META_ARCHITECTURE:
        ...
        if cfg.INPUT.CROP.ENABLED and is_train:
            ...
            self.compute_tight_boxes = False
        self.strong_augmentation = build_strong_augmentation_detect(cfg, is_train)
        ...
        self.seg_instances_only = cfg.SEMISUPNET.SEG_INSTANCES_ONLY
    ...
    def __call__(self, dataset_dict):
        ...
        # convert to PIL format first.
        image_pil = Image.fromarray(image_weak_aug.astype("uint8"), "RGB")
        img_strong, regions = self.strong_augmentation(image_pil)
...
class DatasetMapper_test(DatasetMapper):
    ...
    def __init__(self, cfg, is_train=True, keep_tf_data=True, use_w=False):
        self.augmentation = new_augs(cfg, is_train, use_w=use_w)
        # include crop into self.augmentation
...
class DatasetMapper_instance_segm:
    """
    A callable which takes a dataset dict in Detectron2 Dataset format,
    and map it into a format used by the model.
    ...
    def __init__(
        ...
        *,
        ...
        self.recompute_boxes        = recompute_boxes
        self.seg_instances_only     = seg_instances_only
        # fmt: on
    ...
    def from_config(cls, cfg, is_train: bool = True):
        ...
        ret = {
            ...
            "seg_instances_only": cfg.SEMISUPNET.SEG_INSTANCES_ONLY
        }
...
def new_augs(cfg, is_train, use_w=False):
    """
    ...
    """
    if use_w:
        dino_patch = cfg.INPUT.DINO_PATCH_SIZE
        if is_train:
            ...
            sample_style = "choice"
        augmentation = [ResizeTransformDinoScaleW(max_size, dino_patch, sample_style)]
    else:
        dino_patch = cfg.INPUT.DINO_PATCH_SIZE
        if is_train:
            min_size = cfg.INPUT.MIN_SIZE_TRAIN
