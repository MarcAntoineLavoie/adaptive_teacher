...
class AspectRatioGroupedSemiSupDatasetTwoCrop_detect(AspectRatioGroupedDataset):
    """
    Batch data that have similar aspect ratio together.
    In this implementation, images whose aspect ratio < (or >) 1 will
    be batched together.
    ...
    def __init__(self, dataset, batch_size):
        ...
        self._label_buckets = [[] for _ in range(2)]
        self._label_buckets_key = [[] for _ in range(2)]
        self._label_buckets_regions = [[] for _ in range(2)]
        self._unlabel_buckets = [[] for _ in range(2)]
        self._unlabel_buckets_key = [[] for _ in range(2)]
        self._unlabel_buckets_regions = [[] for _ in range(2)]
        # Hard-coded two aspect ratio groups: w > h and w < h.
        # Can add support for more aspect ratio groups, but doesn't seem useful
    ...
    def __iter__(self):
        ...
        for d_label, d_unlabel in zip(self.label_dataset, self.unlabel_dataset):
            ...
            if len(label_bucket) != self.batch_size_label:
                ...
                label_buckets_key = self._label_buckets_key[label_bucket_id]
                label_buckets_key.append(d_label[1])
                label_buckets_regions = self._label_buckets_regions[label_bucket_id]
                label_buckets_regions.append(d_label[2])
            ...
            if len(unlabel_bucket) != self.batch_size_unlabel:
                ...
                unlabel_buckets_key = self._unlabel_buckets_key[unlabel_bucket_id]
                unlabel_buckets_key.append(d_unlabel[1])
                unlabel_buckets_regions = self._unlabel_buckets_regions[unlabel_bucket_id]
                unlabel_buckets_regions.append(d_unlabel[2])
            ...
            if (
                ...
                yield (
                    label_bucket[:],
                    label_buckets_key[:],
                    label_buckets_regions[:],
                    unlabel_bucket[:],
                    unlabel_buckets_key[:],
                    unlabel_buckets_regions[:],
                )
                del label_bucket[:]
                del label_buckets_key[:]
                del label_buckets_regions[:]
                del unlabel_bucket[:]
                del unlabel_buckets_key[:]
                del unlabel_buckets_regions[:]
